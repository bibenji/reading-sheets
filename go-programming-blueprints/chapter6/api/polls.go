...
type poll struct {
	ID      bson.ObjectId  `bson:"_id" json:"id"` // we specify field name for encoding and decoding
...
func (s *Server) handlePolls(w http.ResponseWriter, r *http.Request) {
	...
	case "DELETE":
		...
		return
	case "OPTIONS":
		// normally it changes in response to the request made
		// but here DELETE is the only case we are supporting
